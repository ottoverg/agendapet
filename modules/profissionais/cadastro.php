...
$page_title = 'Cadastrar Profissional';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!validaCPF($cpf)) {
        ...
        $sql = "SELECT id FROM profissionais WHERE cpf = ?";
        $exists = fetchOne($sql, [$cpf]);
        
        if ($exists) {
            $_SESSION['error_message'] = 'Já existe um profissional cadastrado com este CPF.';
        } else {
            // Inserir no banco de dados
            $sql = "INSERT INTO profissionais 
                    (nome, cpf, cep, logradouro, numero, complemento, bairro, cidade, uf, 
                     telefone, email, banhista, tosador, observacoes, data_cadastro)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW())";
            $params = [$nome, $cpf, $cep, $logradouro, $numero, $complemento, $bairro, 
                      $cidade, $uf, $telefone, $email, $banhista, $tosador, $observacoes];
            
            try {
                executeQuery($sql, $params);
                $_SESSION['success_message'] = 'Profissional cadastrado com sucesso!';
                header('Location: listar.php');
                exit();
            } catch (PDOException $e) {
                $_SESSION['error_message'] = 'Erro ao cadastrar profissional: ' . $e->getMessage();
            }
        }
    }
}
...
<div class="container mt-4">
    <div class="row">
        <div class="col-md-12">
            <h2><i class="fas fa-user-md"></i> Cadastrar Novo Profissional</h2>
            <hr>
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <div class="form-check">
                                <input class="form-check-input" type="checkbox" id="banhista" name="banhista" value="1">
                                <label class="form-check-label" for="banhista">Banhista</label>
                            </div>
                            <div class="form-check">
                                <input class="form-check-input" type="checkbox" id="tosador" name="tosador" value="1">
